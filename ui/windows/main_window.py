...
class MainWindow:
    ...
    def show_update_notification(self, manager_update_info, zapret_update_info):
        ...
        _d_layout = float(dampened_hi_dpi_factor(warning_dialog_scale(self.root)))
        _d_font = float(dampened_hi_dpi_factor(logical_ui_scale(self.root)))
        ...
        def _apply_update_scale(ff_l: float) -> None:
            ff_t = float(dampened_hi_dpi_factor(logical_ui_scale(self.root)))
            ...
            try:
                ...
                center_toplevel_on_parent(notification_window, self.root)
            except tk.TclError:
                pass

        def _current_scale_tuple():
            nu = float(warning_dialog_scale(self.root))
            nl = float(logical_ui_scale(self.root))
            try:
                ntk = float(notification_window.tk.call("tk", "scaling", "-displayof", notification_window))
            except Exception:
                ntk = None
            try:
                dtk = winfo_dpi(self.root)
        ...
        def _try_refit_update_for_scale() -> bool:
            ...
            _ff_ap = float(dampened_hi_dpi_factor(warning_dialog_scale(self.root)))
